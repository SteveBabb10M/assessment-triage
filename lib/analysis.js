...
async function aiAnalysis(text, studentName, assignmentId, uploaderName = null, studentCohortId = null, forensicsData = null, fileType = 'docx', pdfBase64 = null) {
  ...
  const isTLevel = assignment.course === 'tlevel-management';
  const criteriaList = assignment.criteria.join(', ');
  const resourceData = loadResources(assignmentId, uploaderName, studentCohortId);
  const resourcesPrompt = resourceData
    ? formatResourcesForPrompt(resourceData.resources, resourceData.teacherName, isTLevel ? 6000 : 4000, resourceData.isTLevel || false)
    : isTLevel
      ? '\nNo T Level marking grids available. Assess against general T Level Level 3 expectations using band-based marking.\n'
      : '\nNo teacher guidance resources available for this assignment. Assess based on BTEC Level 3 general expectations.\n';
  const resourcesNote = resourceData ? (isTLevel
    ? `\nIMPORTANT: T Level ESP marking grids and reference documents are provided below (from ${resourceData.teacherName}). You MUST use the band descriptors to assess this submission. Do NOT use BTEC-style Pass/Merit/Distinction grading.\n`
  ...
  // Build the criteria/grading section based on qualification type
  let gradingPrompt;
  if (isTLevel) {
    const gridsDescription = assignment.grids
      ? assignment.grids.map((g, i) => `  Grid ${i + 1}: ${g.name} — ${g.maxMarks} marks (${g.bands} bands)`).join('\n')
      : '  (Grid details not available — estimate based on general band descriptors)';

    gradingPrompt = `
═══════════════════════════════════════════════════════
PART B: QUALITY ASSESSMENT — T LEVEL ESP BAND-BASED MARKING
═══════════════════════════════════════════════════════

CRITICAL: This is a T Level Employer-Set Project, NOT a BTEC assignment. Do NOT use Pass/Merit/Distinction criteria. This qualification uses NUMERICAL MARKS against BAND DESCRIPTORS.

This task has the following marking grids (total ${assignment.maxMarks} marks):
${gridsDescription}

For EACH marking grid:
1. Read the band descriptors from the marking grid resource provided
2. Assess the student's work against each band descriptor holistically
3. Determine the best-fit band
4. Estimate a specific mark within that band's range
5. Provide brief justification referencing the band descriptor language

The A-grade exemplar (if provided in resources) scored ${assignment.id === 'tl-1-1' ? '15/20' : assignment.id === 'tl-1-2' ? '14/17' : 'varies'} — use this as a calibration benchmark.

GRADING OUTPUT: Provide estimated marks per grid AND a total out of ${assignment.maxMarks}. The overall grade descriptor should be one of:
- "Band 3-4 (Strong)" for work scoring in the upper bands across most grids
- "Band 2-3 (Competent)" for work sitting mostly in middle bands
- "Band 1-2 (Developing)" for work in the lower bands
- "Band 1 (Limited)" for work barely meeting the lowest band descriptors
- "Below Band 1" for work with insufficient evidence

If there are originality concerns from Part A, note that marks are conditional on verification — but do NOT let originality concerns change the mark estimates themselves.

RECOMMENDED NEXT STEPS (for the student):
After completing the mark assessment, generate a "recommendedNextSteps" array. Rules:
- Focus on the grids where the student scored lowest relative to the available marks.
- Write each step as if speaking directly to the student. Use simple, clear English appropriate for T Level students who may have English as an additional language.
- Be SPECIFIC: reference what they actually wrote and what specifically would move them into the next band.
- Include a sentence starter or example phrase where it would help the student get started.
- If the student scored in the top band across all grids, return an empty array.
`;
  } else {
    gradingPrompt = `
═══════════════════════════════════════════════════════
...
RECOMMENDED NEXT STEPS (for the student):
After completing the criteria assessment, generate a "recommendedNextSteps" array. Rules:
- ONLY include criteria that are NOT "Met" — do not include advice for criteria the student has already achieved.
- Write each step as if speaking directly to the student. Use simple, clear English appropriate for Level 3 students who may have English as an additional language.
- Be SPECIFIC: reference what they actually wrote and what specifically is missing. Do not give generic advice.
- Include a sentence starter or example phrase where it would help the student get started.
- If the overall grade is Distinction (all criteria Met), return an empty array.
- Order by priority: P criteria first, then M, then D.
`;
  }

  // Build the JSON response schema based on qualification type
  let responseSchema;
  if (isTLevel) {
    responseSchema = `{
  ...
  "originalityAssessment": {
    ...
    "humanizerMarkers": [{"marker": "<description>", "evidence": "<specific evidence>", "significance": "<why this matters>"}],
    "directAIMarkers": [{"marker": "<description>", "evidence": "<specific examples>", "significance": "<why this matters>"}],
    "authenticIndicators": ["<specific evidence of genuine student work>"],
    "vocabularyFlags": [{"quote": "<exact quote>", "concern": "<why beyond expected level>"}],
  ...
  "holisticAssessment": {
    ...
    "gridAssessments": [
      {
        "gridName": "<e.g. AO1 + AO3 (Planning & Techniques)>",
        "maxMarks": <marks available for this grid>,
        "bestFitBand": <band number>,
        "estimatedMark": <specific mark>,
        "justification": "<2-3 sentences referencing band descriptor language>"
      }
    ],
    "totalEstimatedMark": <sum of grid marks>,
    "overallGrade": "<Band 3-4 (Strong)|Band 2-3 (Competent)|Band 1-2 (Developing)|Band 1 (Limited)|Below Band 1>",
    "gradeJustification": "<1-2 sentences explaining the overall mark estimate>",
    "conditionalNote": "<if originality concerns: note marks are conditional on verification>",
    "criteriaAssessments": []
  },
  "recommendedNextSteps": [
    {
      "criterion": "<e.g. AO1 or grid name>",
      "currentStatus": "<current band or mark estimate>",
      "whatToDoNext": "<Written TO the student in simple, accessible English. Be specific about what they need to add, change, or improve to move into the next band. Include a sentence starter or example where helpful. 1-3 sentences max.>"
    }
  ],
  "verificationQuestions": ["<specific questions for teacher>"],
  ...
  "recommendedNextSteps": [
    {
      "criterion": "<e.g. P1>",
      "currentStatus": "<Met|Partially met|Not yet met|Not evidenced>",
      "whatToDoNext": "<Written TO the student in simple, accessible English. Be specific about what they need to add, change, or improve. Include a sentence starter or example where helpful. Keep it short — 1-3 sentences max.>"
    }
  ],
  ...
  const prompt = `You are an experienced ${isTLevel ? 'T Level Management and Administration' : 'BTEC Level 3 Business'} assessor producing a combined assessment report. This report has TWO PARTS that must be assessed INDEPENDENTLY using different approaches.

CONTEXT:
- Student: ${studentName}
- ${isTLevel ? 'T Level ESP' : `Unit ${assignment.unitNumber}`}: ${assignment.unitTitle}
- Assignment: ${assignment.name}
- ${isTLevel ? `Assessment Objectives: ${criteriaList}` : `Criteria being assessed: ${criteriaList}`}
- ${isTLevel ? 'T Level students are 16-18 year olds at Gateway College, Leicester — many are bilingual learners' : 'BTEC Level 3 students are 16-18 year olds, many are bilingual learners at Gateway College, Leicester'}
- ${isTLevel ? `This task is worth ${assignment.maxMarks} marks out of 100 total ESP marks` : 'Students follow a describe > explain > evaluate progression (Pass > Merit > Distinction)'}
...
2. DIRECT AI GENERATION (no humanizer):
   - Harvard-style academic citations atypical for ${isTLevel ? 'T Level' : 'L3 BTEC'}
...
IMPORTANT: Your originality assessment should NOT influence the ${isTLevel ? 'mark estimates' : 'criterion grades'} in Part B. Those are assessed independently. If you find originality concerns, the "conditionalNote" field handles the relationship.
...
export async function analyzeSubmission(text, studentName, assignmentId, adHocUnitNumber = null, uploaderName = null, studentCohortId = null, forensicsData = null, fileType = 'docx', pdfBase64 = null) {
  ...
  function stage3Triage(forensicsData, aiResult, originalityScore) {
    ...
    // For BTEC: Distinction/Merit = high, Referral = low
    // For T Level: Band 2-3/3-4 = high, Band 1/Below = low
    const isTLevelGrade = grade && grade.startsWith('Band');
    const isHighAmplitude = isTLevelGrade
      ? ['Band 3-4 (Strong)', 'Band 2-3 (Competent)'].includes(grade)
      : ['Distinction', 'Merit'].includes(grade);
    const isLowAmplitude = isTLevelGrade
      ? ['Band 1 (Limited)', 'Below Band 1'].includes(grade) || grade === null
      : ['Referral'].includes(grade) || grade === null;
  ...
  try {
    ...
    // For T Level: also store grid assessments and total mark
    if (ai.holisticAssessment?.gridAssessments) {
      ai.holisticAssessment.gridAssessments.forEach((ga, i) => {
        criteriaResults[`grid_${i + 1}`] = `${ga.estimatedMark}/${ga.maxMarks}`;
      });
      if (ai.holisticAssessment.totalEstimatedMark !== undefined) {
        criteriaResults.totalMark = `${ai.holisticAssessment.totalEstimatedMark}/${ai.holisticAssessment.maxMarks || '?'}`;
      }
    }
