// AI Analysis Engine — v4 (Amplitude/Frequency Architecture)
// 
// Architecture:
...
//    b. ORIGINALITY ASSESSMENT — runs WITH forensic lens. Assesses construction character.
...
${resourcesPrompt}
═══════════════════════════════════════════════════════
PART A: ORIGINALITY ASSESSMENT — NOW APPLY FORENSIC CONTEXT
═══════════════════════════════════════════════════════
${forensicLens.lens}
...
export async function analyzeSubmission(text, studentName, assignmentId, adHocUnitNumber = null, uploaderName = null, studentCohortId = null, forensicsData = null) {
  ...
  // Calculate forensic score for storage
  ...
  function stage3Triage(forensicsData, aiResult, originalityScore) {
    ...
    if (!forensicsAvailable) {
      ...
      // RULE 1 & 2: Definitive forensic evidence, OR placed + strong grades = Red
      ...
      // RULE 3: Placed/assembled but weak grades = Amber
      // The forensics say it was placed, but the content didn't meet criteria well.
      // This could mean: poor tool use, cobbled fragments, or benign forensic explanation.
      // Cannot be Green (forensics are objective) but concern is modulated.
      ...
      // Uniform formatting but not definitive — use text analysis with caution
      if (aiOriginalityScore < 50) { classification = 'REVIEW (Red)'; flag = 'red'; }
      ...
      // Low forensic concern — trust the text analysis
      if (aiOriginalityScore < 50) { classification = 'REVIEW (Red)'; flag = 'red'; }
      else if (aiOriginalityScore < 80) { classification = 'CHECK (Amber)'; flag = 'yellow'; }
      else { classification = 'ON TRACK (Green)'; flag = 'green'; }
  ...
  try {
    ...
    const triage = isAdHoc
      ? stage3Triage(forensicsData, null, originalityScore)
      : stage3Triage(forensicsData, ai, originalityScore);
    ...
    const triage = stage3Triage(forensicsData, null, local.originalityScore);
