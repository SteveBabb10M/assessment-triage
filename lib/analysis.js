...
async function aiAnalysis(text, studentName, assignmentId) {
  ...
  const resourceData = loadResources(assignmentId);
  const resourcesPrompt = resourceData ? formatResourcesForPrompt(resourceData.resources) : '\nNo teacher guidance resources available for this assignment. Assess based on BTEC Level 3 general expectations.\n';
  const resourcesNote = resourceData ? `\nIMPORTANT: Teacher guidance resources are provided below. Students had access to: ${resourceData.summary}. Content that follows the scaffolding structure is EXPECTED and should not be flagged as suspicious. However, content significantly beyond the scaffolding, or using language/concepts not in the guidance, warrants closer originality scrutiny.\n` : '';
