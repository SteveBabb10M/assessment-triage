...
import { formatForensicsForPrompt } from './docForensics';
...
async function aiAnalysis(text, studentName, assignmentId, uploaderName = null, studentCohortId = null, forensicsData = null) {
  ...
  const forensicsPrompt = forensicsData ? formatForensicsForPrompt(forensicsData) : '';
...
3. DOCUMENT FORENSICS (formatting metadata — very strong objective evidence):
   - Font origin signatures (AI platforms use different default fonts than Word)
   - Bold/normal inconsistencies in body text (indicates separate paste operations)
   - Font transitions mid-document (Word default → AI platform font)
   - American vs British spelling conventions (ChatGPT defaults to American English)
   - These markers cannot be faked by humanizer tools and students rarely know they exist

4. AUTHENTIC STUDENT INDICATORS:
   ...
   - Consistent font and formatting throughout document (single writing environment)
...
Respond ONLY in valid JSON with no preamble or markdown fencing:
{
  ...
  "originalityAssessment": {
    ...
    "documentForensics": {
      "fontAnalysis": "<summary of font findings — which fonts detected, whether AI platform fonts present>",
      "boldPatternAnalysis": "<summary of bold/normal body text inconsistencies>",
      "spellingConventionAnalysis": "<American vs British spelling findings>",
      "forensicRiskLevel": "<clean|low|medium|high>",
      "forensicVerdict": "<1-2 sentence interpretation of what the formatting evidence means>",
      "pasteOperationsEstimate": <number of estimated separate paste operations based on font/bold transitions>
    },
...
Analyse for: humanizer detection, direct AI generation markers, document forensics (font/formatting evidence), authentic student indicators, copy-paste indicators. Provide specific evidence with quotes and counts.
${forensicsPrompt}
