...
//   Phase 1: Document forensics (mechanical, deterministic)
//   Phase 2: Reasoning and pattern analysis (AI, contextualised by Phase 1 lens)
...
function buildForensicsSection(forensicsData, forensicScore) {
  ...
  if (s.riskLevel === 'clean') {
    ...
    context = 'The document shows consistent formatting throughout, consistent with a single author working in one environment. No indicators of content assembled from external sources.';
  } else if (s.riskLevel === 'low') {
    integrity = 'Consistent';
    context = 'The document shows minor formatting variations that are within normal range for student work. No significant indicators of external content.';
  } else if (s.riskLevel === 'medium') {
    integrity = 'Some concerns';
    context = buildContextSummary(forensicsData, flags, 'medium');
  } else {
    integrity = 'Atypical';
    context = buildContextSummary(forensicsData, flags, 'high');
  }
...
function buildContextSummary(forensicsData, flags, level) {
  ...
  // What does the document behaviour tell us?
  ...
  // Even if no specific flags, a repasted document is itself notable
