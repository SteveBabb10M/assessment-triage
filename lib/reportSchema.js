...
export function buildReport(metadata, forensicsData, aiAnalysisResult) {
  ...
  return {
    ...
    criteriaAssessment: metadata.isAdHoc ? null : {
      ...
      // T Level grid assessments (band-based marking)
      gradingType: ha.gradingType || 'criteria',
      maxMarks: ha.maxMarks || null,
      totalEstimatedMark: ha.totalEstimatedMark || null,
      gridAssessments: (ha.gridAssessments || []).map(ga => ({
        gridName: ga.gridName || '',
        maxMarks: ga.maxMarks || 0,
        bestFitBand: ga.bestFitBand || null,
        estimatedMark: ga.estimatedMark || 0,
        justification: ga.justification || '',
      })),
    },
    ...
    // ── Recommended Next Steps (student-facing) ──
    // Only for standard submissions with unmet criteria.
    // Written to the student in accessible language.
    recommendedNextSteps: metadata.isAdHoc ? [] : (aiAnalysisResult?.recommendedNextSteps || []),
  };
}
